...
# 远程live版本号
remote_live_version_url = os.path.join(remote_host, app_name, "live_list.txt")
# 保存的版本数
versions_to_keep = 3
# 下载文件和解压后文件存放目录
...
def getRemoteLiveVersion():
    ...
    req = urllib2.urlopen(remote_live_version_url)
    for i in req:
        remote_live_version = i.strip()
    return remote_live_version.split()[0]
